...
public:
  ...
  bool remove (int value) {
    ...
    while (ptrAtual->next != nullptr && ptrAtual->value < value){
      ptrAnt = ptrAtual;
      ptrAtual = ptrAtual->next;
    }
    ...
    if(ptrAtual->value == value){
      ...
      if(ptrAnt){
        ...
        if (ptrAtual->next) ptrAtual->next->previous = ptrAnt;
        else last = ptrAtual->previous;
      } else{
        first = ptrAtual->next;
        if (ptrAtual->next) first->previous = nullptr;
        else last = nullptr;
      }
